...
class Bot(commands.AutoShardedBot):
    ...
    async def start_tasks(self):
        ...
        # Start WebGUI Dashboard
        try:
            from webgui import start_webgui_async
            webgui_host = config("WEBGUI_HOST", default="0.0.0.0")
            webgui_port = int(config("WEBGUI_PORT", default="8080"))
            logging.info(f"Starting WebGUI Dashboard on http://{webgui_host}:{webgui_port}")
            asyncio.create_task(start_webgui_async(bot, webgui_host, webgui_port))
        except Exception as e:
            logging.warning(f"Failed to start WebGUI Dashboard: {e}")
            logging.warning("Install requirements: pip install psutil uvicorn fastapi")
        
        logging.info("All tasks are now running!")


# Define environment before it's used
